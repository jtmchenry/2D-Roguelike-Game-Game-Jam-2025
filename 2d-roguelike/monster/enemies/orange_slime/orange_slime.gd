extends Enemy

@onready var animated_sprite = $OrangeSlimeAnimatedSprite

@export var projectile_scene: PackedScene
@export var shoot_speed: float = 200.0
@export var charge_time: float = 1.5
@export var wander_time: float = 2.0

var wander_direction: Vector2 = Vector2.ZERO

@onready var wander_timer: Timer = $WanderTimer
@onready var shoot_timer: Timer = $ShootTimer


func _ready():
	damage_timer.connect("timeout", _damage_player)
	add_to_group("Enemies")
	if not player:
		var players = get_tree().get_nodes_in_group("player")
		player = players[0]
	state_machine = $StateMachine

func _physics_process(delta: float):
	var direction = player.global_position - global_position
		
	if direction.x > 0:
		animated_sprite.flip_h = false
	elif direction.x < 0:
		animated_sprite.flip_h = true
		
func take_damage(damage: int, critical: bool):	
	is_hurt = true
	animated_sprite.play("Hurt")
	if critical:
		await $Health.hit(damage, Color.MEDIUM_PURPLE);
	else:
		await $Health.hit(damage, Color.WHITE);
	if($Health.health <= 0):
		AudioManager.play_sfx("explosion")
		
func shoot():
	if player and projectile_scene:
		var projectile = projectile_scene.instantiate()
		get_parent().add_child(projectile)
		projectile.global_position = global_position
		var dir = (player.global_position - global_position).normalized()
		projectile.setup(dir * shoot_speed)

func _on_area_2d_body_entered(body: Node2D) -> void:
	if (_is_player(body)):
		_damage_player()
		damage_timer.start()

func _on_area_2d_body_exited(body: Node2D) -> void:
	if(_is_player(body)):
		damage_timer.stop()

func _on_orange_slime_animated_sprite_animation_finished() -> void:
	if animated_sprite.animation == "Hurt":
		is_hurt = false
		animated_sprite.play("Idle")
	if animated_sprite.animation == "Spawning":
		is_spawning = false
		animated_sprite.play("Idle")
		state_machine.change_state(get_node("StateMachine/Wander"))
